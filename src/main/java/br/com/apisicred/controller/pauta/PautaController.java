...
import br.com.apisicred.controller.pauta.dto.PautaResultadoDto;
...
public class PautaController extends AbsrtactController {
	...
	@GetMapping("/{idPauta}")
	@ResponseStatus(value = HttpStatus.OK)
	public ResponseEntity<PautaDto> findById(@PathVariable("idPauta") Integer idPauta)
			throws BussinesExceptionNotFound {
	...
	public List<PautaResultadoDto> getPautasComResultado() {
		return pautaService.findAll().stream().map(this::getPauta).collect(Collectors.toList());
	}

	@GetMapping("/resultado/{idPauta}")
	public PautaResultadoDto getPautaComResultado(@PathVariable("idPauta") Integer idPauta) {
		return getPauta(pautaService.findById(idPauta));
	}

	private PautaResultadoDto getPauta(Pauta pauta) {
		PautaResultadoDto pautaResponse = conversorObjetos(pauta, PautaResultadoDto.class);
		pautaResponse.setResultadoEleicao(pautaService.resultadoEleicao(pauta));
